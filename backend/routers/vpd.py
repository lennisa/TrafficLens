"""
Vehicle & Person Detection — BLAZING FAST VERSION
...
from backend.inference import get_vpd_model
...
def _get_class_labels(model):
    ...
    if hasattr(model, "session"):
        try:
            ...
            if "names" in meta:
                mapping = ast.literal_eval(meta["names"])
                mapping[7] = "pedestrian"
                return mapping
        except Exception:
            pass
    return {}
...
def _postprocess_onnx_ultralytics(raw_output, meta, orig_shape, model):
    ...
    cls_probs = preds[:, 4:]     # Probabilities (YOLOv8 ONNX exports already apply sigmoid)
    ...
    # Prepare boxes for NMS (OpenCV NMSBoxes expects [x, y, w, h])
    nms_boxes = []
...
@router.post("/image")
async def vpd_image(request: Request, file: UploadFile = File(...)):
    suffix = Path(file.filename).suffix.lower()
    tmp = await save_upload_async(file, suffix)
    try:
        sess = new_session("vpd", file.filename)
        model = get_vpd_model()
...
async def vpd_image_annotated(file: UploadFile = File(...)):
    suffix = Path(file.filename).suffix.lower()
    tmp = await save_upload_async(file, suffix)
    try:
        model = get_vpd_model()
...
@router.post("/video", summary="Stream VPD inference on an uploaded video")
async def vpd_video(
    request: Request,
...
async def vpd_demo_video():
    for ext in [".mp4", ".avi", ".mov"]:
        p = settings.DEMO_DIR / f"vpd{ext}"
        if p.exists():
            sess = new_session("vpd", f"demo{ext}")
            model = get_vpd_model()
...
async def vpd_demo_image(request: Request):
...
async def vpd_demo_image_annotated():
    for ext in [".jpg", ".jpeg", ".png"]:
        p = settings.DEMO_DIR / f"vpd{ext}"
        if p.exists():
            model = get_vpd_model()
...
@router.get("/sessions")
async def vpd_sessions():
    return safe_json_response(list_sessions(task="vpd"))


@router.get("/metrics")
async def vpd_metrics():
    model = get_vpd_model()
    class_labels = _get_class_labels(model)
    if isinstance(class_labels, dict):
