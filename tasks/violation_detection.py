"""
tasks/violation_detection.py
YOLOv11 Violation Detection
...
def draw_detections(frame: np.ndarray, detections: List[Dict], copy: bool = True) -> np.ndarray:
    ...
    try:
        ...
        for d in detections:
            ...
            color = CLASS_COLORS.get(d.get("class_name", ""), (200, 200, 200))
            cv2.rectangle(img, (x1, y1), (x2, y2), color, bk)
            cv2.putText(img, f"{d.get('class_name','')} {d.get('confidence',0)*100:.0f}%",
...
class ViolationDetector:
    ...
    def _postprocess(self, raw, meta, frame) -> List[Dict]:
        ...
        traffic_mask = np.array([cid in COCO_TRAFFIC_CLASSES for cid in class_ids])
        ...
        return [
            {
                ...
                "class_name": COCO_TRAFFIC_CLASSES.get(int(class_ids[i]), "unknown"),
            }
            for i in keep
        ]
